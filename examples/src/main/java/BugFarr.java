...
public class BugFarr {
    ...
    throws Exception {
        ...
        Database db = new DataAPIClient(ASTRA_TOKEN,
          new DataAPIClientOptions()).getDatabase(ASTRA_ENDPOINT);
    ...
    public static void runQueryCollectionBm25(Database db, String collection, String term) {
        ...
        db.getCollection(collection)
          .find(Filters.match(term), new CollectionFindOptions().logRequest())
          .toList()
          .forEach(System.out::println);
    ...
    private static void runQueryFindAndRerank(Database db, String colOpenai, String vQuery, String term) {
        ...
        Hybrid hybrid = new Hybrid().vectorize(vQuery).lexical(term);
        //Sort.hybrid(vQuery, term)
        ...
        db.getCollection(colOpenai)
          ...
          .toList()
          .forEach(rr -> {
              System.out.println("Reranking " + rr.getDocument());
          });
    }
