package com.datastax.astra.test.tables;
...
//        });
    ...
    public void should_drop_type() {
        getDatabase().listTypeNames();
        getDatabase().listTypes();
    ...
    @Test
    public void should_update_non_string_1() {
        Database db = getDatabaseVector();
        Table<Row> tableExo = db.getTable("exotic");
        TableUpdateOperation update = new TableUpdateOperation().set("exotic_map_1",
             Arrays.asList(Arrays.asList(1, "value1"), Arrays.asList(2, "value2")));
        tableExo.updateOne(Filters.eq("name", "entry2"), update);
    }

    @Test
    public void should_update_non_string_2() {
        Database db = getDatabaseVector();
        Table<Row> tableExo = db.getTable("exotic");
        TableUpdateOperation update = new TableUpdateOperation()
                .set("exotic_map_1", Map.of(1, "value1",2, "value2"));
        tableExo.updateOne(Filters.eq("name", "entry2"), update);
    }
    ...
    public static final class ExoticTableRowAnnotated {
        String name;
        @JsonProperty("normal_map")
        Map<String, String> normalMap;
        @JsonProperty("exotic_map_1")
    ...
    // Should work with composition of UDT
    ...
    public void should_create_table_with_composite_udt() {
        getDatabase().createType(Person.class, CreateTypeOptions.IF_NOT_EXISTS);
        getDatabase().createTable(Group.class, IF_NOT_EXISTS);
    }

    @Test
    public void should_not_fail_list_indexes() {
        // Create Table
//        Table<Row> table = getDatabase().createTable("t_lexical_demo", new TableDefinition()
//                .addColumnText("email")
//                .addColumnText("name")
//                .partitionKey("email"), CreateTableOptions.IF_NOT_EXISTS);

        // Create Text Index
        //table.createTextIndex("idx_name2", "name");
        getDatabase().getTable("t_lexical_demo")
          .listIndexes()
