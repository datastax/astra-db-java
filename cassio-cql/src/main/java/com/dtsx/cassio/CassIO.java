...
public class CassIO {
     ...
     *    the cassandra session initialized
     */
    public static synchronized CqlSession init(String token, String scbPath, String keyspace) {
        cqlSession = CqlSession.builder()
                .withAuthCredentials("token", token)
                .withCloudSecureConnectBundle(Paths.get(scbPath))
    ...
    public static synchronized CqlSession init(String token, UUID dbId, String dbRegion, String keyspace, AstraEnvironment env) {
        String secureConnectBundleFolder = System.getenv("ASTRA_DB_SCB_FOLDER");
        if (secureConnectBundleFolder == null) {
            secureConnectBundleFolder = System.getProperty("user.home") + File.separator + ".astra" + File.separator + "scb";
        }
