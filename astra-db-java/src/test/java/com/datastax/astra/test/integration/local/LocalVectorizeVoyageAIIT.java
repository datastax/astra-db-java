...
/**
 * Vectorize integration tests using VoyageAI embeddings against local HCD/DSE.
 * <p>
 * Tests require:
 * <ul>
 *   <li>Local HCD or DSE instance running with vectorize enabled</li>
 *   <li>VoyageAI API key configured in test-config.properties or VOYAGE_API_KEY env var</li>
 * </ul>
 */
@EnabledIfLocalAvailable
@Slf4j
@DisplayName("Local Vectorize VoyageAI Integration Tests")
public class LocalVectorizeVoyageAIIT extends AbstractVectorizeApiHeaderIT {
    ...
    protected String getApiKey() {
        return getConfig().getVoyageApiKey();
    }
    ...
    public void should_test_vectorize_with_api_header() {
        if (!getConfig().hasVoyageApiKey()) {
            log.info("Skipping test - VoyageAI API key not configured");
            return;
        }
